...
test("user can create a source post draft and see it in the library", async ({ page }) => {
  ...
  await page.getByLabel("Source URL").fill("https://example.invalid/community/thread-99");
  await page.getByRole("button", { name: "Import Source" }).click();
  await expect(page.getByText("https://example.invalid/community/thread-99")).toBeVisible();
